...
int fileNotExist(FILE*, FILE*, FILE*, FILE*);
void fillData(Clients_st*, Aminities_st*, Used_aminities_st*, FILE*, FILE*, FILE*);
void fillClients(Clients_st*, FILE*);
void fillAminities(Aminities_st*, FILE*);
void fillUsedAminities(Used_aminities_st*, FILE*);
void convertTimeOfUsing(Used_aminities_st*);
int CharToInt32(char*);
int Compare(char*, char*);
int compareDate(int, int, char*);
...
int main()
{
	...
	fillData(people, amin, used_amin, clients, aminities, used_aminities); // Filling structures with data.

	fclose(clients);
	fclose(aminities);
	fclose(used_aminities);

	convertTimeOfUsing(used_amin);//Converting data from string to int32
...
void fillClients(Clients_st *people, FILE *clients)
{
	...
	while (!feof(clients)) {
		...
		k++;
		s = 0;
		//printf("%s %s %s %s %s %s\n", people[sch].name, people[sch].t_number, people[sch].date_s, people[sch].date_e, people[sch].bill, people[sch].credit);
		sch++;
		k = 0;
	}
	people[1].count = sch;
...
void convertTimeOfUsing(Used_aminities_st *used_amin)
{
...
int compareDate(int m, int y, char *b)
{
...
void Report(char *ami1, char *ami2, Clients_st *clients, Aminities_st *aminities, Used_aminities_st *used_aminities, FILE *report)
{
	...
	for (int i = 0; i < clients[1].count; i++)
	{
		...
		{
			if (Compare(used_aminities[j].t_number, clients[i].t_number) && (Compare(used_aminities[j].code, ami1) ||Compare(used_aminities[j].code, ami2)) && compareDate(month, year, used_aminities[j].date))
			{
